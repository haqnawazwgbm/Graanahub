  ...
  <div class="content container-fluid">
					<div class="row">
						...
						<div class="col-xs-8 text-right m-b-30">
							<a href="#" id="view_calendar" class="btn btn-info add-form-btn"><i class="fa fa-calendar"></i> View Calendar</a>
						</div>
					</div>
									...
									<tbody>
										...
										<tr>
											...
											<td><div class="dropdown action-label">
													...
													<a class="btn btn-white btn-sm rounded view_details" href="#" path="<?= base_url().'Requests/to_manager_request_details/'.$request['request_id']; ?>" aria-expanded="false"><i class="fa fa-eye text-success"></i> View Details 
													</a>
												</div></td>
											<td><div class="dropdown action-label">
												<?php if ($request['approved_by_manager'] == 0 && $request['status'] == 1) : ?>
												...
												<?php elseif ($request['approved_by_manager'] != 0 && $request['status'] == 1) : ?>
												...
												<?php endif; ?>
													<ul class="dropdown-menu pull-right">
														...
														<li><a href="#" class="edit_record" path="<?= base_url().'Requests/edit_form/'.$request['request_id']; ?>" class="reject_request"><i class="fa fa-pencil"></i> Edit</a></li>
													</ul>
												</div>
											</td>
										</tr>
	...
	$(document).ready(function() {
		$('.approve_request').click(function() {
			...
			$('#view_calendar').on('click', function() {
        			$.ajax( {
        				url: "<?= base_url(); ?>Requests/view_calendar",
        				type: 'GET',
        				success: function(data) {
        					$('.edit-form-container').html(data);
        					$('.edit-form-container').find('#calendar_modal').modal('show');
        					return false;
        				}
        			})
        		})
